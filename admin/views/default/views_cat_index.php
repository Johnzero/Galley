...
<div id="page_string" class="form-inline definewidth m10"></div>
...
function common_request(){
	var url="<?php echo site_url("cat/index");?>?inajax=1";
	var data_ = {
		'page':page,
		'time':<?php echo time();?>,
		'action':'ajax_data',
	...
	$.ajax({
		 ...
		 //  async:false,
			...
			if(msg.resultcode<0){
				...
				if(list.length>0){
					for(var i in list){
						var onclick = '' ;					
						shtml+='<tr>';
						shtml+='<td >'+list[i]['html']+''+list[i]['typename']+'</td>';				
						shtml+='<td>'+list[i]['seotitle']+'</td>';
						shtml+='<td>'+list[i]['keywords']+'</td>';
						shtml+='<td>'+list[i]['html']+''+list[i]['disorder']+'</td>';
						shtml+='<td><a href="javascript:void(0);" onclick="add_('+list[i].id+')" class="icon-plus" title="添加'+list[i]['typename']+'的子类"></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="edit('+list[i].id+','+list[i].pid+',\''+list[i].typename+'\')" class="icon-edit" title="编辑'+list[i]['typename']+'"></a></td>';
						shtml+='</tr>';
					}
					$("#result_").html(shtml);				
		   ...
		   beforeSend:function(){
			  $("#result_").html('<font color="red"><img src="/static/images/progressbar_microsoft.gif"></font>');
		   },
		   error:function(){
...
function edit(id,mypid,name){
	...
	var dialog = new Overlay.Dialog({
		...
		height:500,
		loader : {
		url : '<?php echo site_url("cat/edit");?>',
...
function submit_edit(id,mypid){
	...
	$.ajax({
		   type: "POST",
		   url: "<?php echo site_url('cat/edit');?>?inajax=1" ,
		   data: data_,
		   cache:false,
		   dataType:"json",
		   async:false,
...
function submit_add(_id){
	...
	$.ajax({
		   type: "POST",
		   url: "<?php echo site_url('cat/add');?>?inajax=1" ,
		   data: data_,
		   cache:false,
		   dataType:"json",
		   async:false,
