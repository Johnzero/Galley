...
class Common extends CI_Controller{
	...
	function Common(){
		...
		$this->table_ =table_pre('real_data')."galley_";
	...
	function get_menu(){
		...
		//查询菜单列表
		$list = $this->M_common->querylist("SELECT id,name,pid as parentid,url,status,addtime,disorder ,url_type,collapsed from {$this->table_}common_admin_nav where status = 1  order by disorder,id desc ");
		$result = array();
		if($list){
			foreach($list as $k=>$v){
				$result[$v['id']]  = $v ;
